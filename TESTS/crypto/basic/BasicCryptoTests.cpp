...
const unsigned char *testPubKey =  new const unsigned char[crypto_sign_PUBLICKEYBYTES]  \
                                {0x13 , 0x25 , 0x81 , 0x6B , 0x2A , 0x20 , 0xF4 , 0xC3, \
                                 0xC1 , 0xC5 , 0x26 , 0x7D , 0x0D , 0xC6 , 0xCF , 0xF5, \
                                 0xFC , 0xE4 , 0xB8 , 0xA0 , 0x45 , 0x85 , 0x3D , 0x62, \
                                 0xF8 , 0xC7 , 0x23 , 0x9D , 0xF0 , 0x3F , 0x85 , 0x2E};

const unsigned char *testSecKey =  new const unsigned char[crypto_sign_SECRETKEYBYTES]  \
                                {0x88 , 0x42 , 0x9A , 0xD2 , 0x50 , 0x44 , 0x55 , 0x54, \
                                 0xD4 , 0xC0 , 0x9A , 0x9A , 0xA1 , 0x6C , 0xC9 , 0x92, \
                                 0xF3 , 0x50 , 0xD4 , 0x6D , 0xA0 , 0x8F , 0x58 , 0x8F, \
                                 0x41 , 0xE8 , 0x3E , 0xF2 , 0xAF , 0x09 , 0xCA , 0xA0, \
                                 0x13 , 0x25 , 0x81 , 0x6B , 0x2A , 0x20 , 0xF4 , 0xC3, \
                                 0xC1 , 0xC5 , 0x26 , 0x7D , 0x0D , 0xC6 , 0xCF , 0xF5, \
                                 0xFC , 0xE4 , 0xB8 , 0xA0 , 0x45 , 0x85 , 0x3D , 0x62, \
                                 0xF8 , 0xC7 , 0x23 , 0x9D , 0xF0 , 0x3F , 0x85 , 0x2E};
...
void TestCryptoImportKey() {
    ...
    TEST_ASSERT_EQUAL_HEX8_ARRAY_MESSAGE(testPubKey, testKey, crypto_sign_PUBLICKEYBYTES ,"data read does not match written data");
}
...
void TestCryptoEncodeDecodeMessage(){
    ...
    for (int i = 0; i < messageLength; i++) {
        origMessage[i] = (unsigned char)(rand() & 0xFF);
    }
    // encode the message
    ...
    TEST_ASSERT_EQUAL_HEX8_ARRAY_MESSAGE(origMessage, decMessage, messageLength ,"data read does not match written data");

    delete origMessage;
}

void TestCryptoSigningVerifyingMessage(){
    // create message and fill it with random bytes
    uint16_t messageLength = 100;
    unsigned char *origMessage = new unsigned char[messageLength];
    for (int i = 0; i < messageLength; i++) {
...
Case cases[] = {
        Case("Crypto test import backendKey-0", TestCryptoImportKey, greentea_failure_handler),
        ...
        Case("Crypto test sign and verify message-1", TestCryptoSigningVerifyingMessage, greentea_failure_handler),
        Case("Crypto test sign and verify message-2", TestCryptoSigningVerifyingMessage, greentea_failure_handler),
        Case("Crypto test sign and verify message-3", TestCryptoSigningVerifyingMessage, greentea_failure_handler),
        Case("Crypto test sign and verify message-4", TestCryptoSigningVerifyingMessage, greentea_failure_handler),
        Case("Crypto test sign and verify message-5", TestCryptoSigningVerifyingMessage, greentea_failure_handler),
        Case("Crypto test sign and verify message-6", TestCryptoSigningVerifyingMessage, greentea_failure_handler),
        Case("Crypto test sign and verify message-7", TestCryptoSigningVerifyingMessage, greentea_failure_handler),
        Case("Crypto test sign and verify message-8", TestCryptoSigningVerifyingMessage, greentea_failure_handler),
};
